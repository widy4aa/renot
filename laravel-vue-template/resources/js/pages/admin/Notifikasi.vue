    ...
    <div class="space-y-5">
        ...
        <div v-else class="space-y-2">
            <div
                ...
                :key="notif.id"
                class="card-elevated rounded-xl px-4 py-3.5 flex gap-3 transition-all"
            ...
            >
                ...
                >
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" :style="{ color: typeConfig(notif.type).accentColor }">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs font-medium leading-relaxed mb-2" style="color:#6B7280;">{{ notif.body }}</p>
                    ...
                    <div class="flex items-center gap-3 flex-wrap">
                        ...
                        <span v-if="!notif.is_read" class="ml-auto w-2 h-2 rounded-full shrink-0" :style="{ background: typeConfig(notif.type).accentColor }"></span>
                    </div>
                </div>
            </div>
        </div>
...
const typeMap = {
    reminder_akan_expired: {
        label: 'Reminder', accentColor: '#D97706', bgColor: '#FEF3C7',
        icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z',
    },
    dokumen_expired: {
        label: 'Expired', accentColor: '#ED1B2F', bgColor: '#FEE2E2',
        icon: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z',
    },
    dokumen_pending: {
        label: 'Pending', accentColor: '#6B7280', bgColor: '#F3F4F6',
        icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2',
    },
    dokumen_approved: {
        label: 'Disetujui', accentColor: '#ACC42A', bgColor: '#F7FEE7',
        icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z',
    },
    dokumen_ditolak: {
        label: 'Ditolak', accentColor: '#DB2777', bgColor: '#FCE7F3',
...
function typeConfig(type) {
    return typeMap[type] ?? {
        label: 'Info', accentColor: '#006CB8', bgColor: '#EFF6FF',
...
useFaq([
    { q: 'Notifikasi apa yang diterima admin?', a: 'Admin menerima notifikasi dokumen pending approval baru, dokumen yang sudah expired, dan reminder dokumen pegawai yang akan kadaluarsa.', icon: 'M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9' },
    { q: 'Cara approve dokumen dari notifikasi?', a: 'Klik notifikasi "Pending" untuk melihat detail dokumen, lalu gunakan tombol Setujui atau Tolak di halaman Approval.', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
    { q: 'Cara mengatur jadwal reminder?', a: 'Buka menu Pengaturan Sistem di sidebar, lalu atur jadwal H-X sesuai kebutuhan. Perubahan berlaku untuk reminder berikutnya.', icon: 'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z' },
]);
</script>
